...
function doIt() {
    if [ "$1" "==" "--dry-run" ]; then
	    ...
	    find . -type f -not -path "*/.git/*" -not -path "./.DS_Store" -not -path "./.osx" -not -path "./bootstrap.sh" -not -path "*/node_modules/*" -not -path "./README.md" -not -path "./LICENSE-MIT.txt" -exec sh -c 'file="{}"; mkdir -pv $HOME/${file%/*}/; ln -fsv ~/dotfiles/$file ~/$file' \;
    else
