...
function doIt() {
    if [ "$1" "==" "--dry-run" ]; then
        ...
        echo "forced"
        find . -type f -not -path "*/.git/*" -not -path "./.DS_Store" -not -path "./.osx" -not -path "./bootstrap.sh" -not -path "./README.md" -not -path "./LICENSE-MIT.txt" -exec mkdir -p $(dirname {}) \; -exec ln --force --symbolic --verbose ~/dotfiles/'{}' ~/'{}' \;
    else
